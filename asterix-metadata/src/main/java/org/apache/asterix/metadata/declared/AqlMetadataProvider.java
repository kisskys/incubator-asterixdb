...
public class AqlMetadataProvider implements IMetadataProvider<AqlSourceId, String> {
        ...
        try {
            ...
            if (dataset.getDatasetType() == DatasetType.INTERNAL) {
                ...
                ExternalBTreeWithBuddyDataflowHelperFactory indexDataflowHelperFactory = new ExternalBTreeWithBuddyDataflowHelperFactory(
                        ...
                        getStorageProperties().getBloomFilterFalsePositiveRate(), buddyBTreeFields,
    ...
    private Pair<IBinaryComparatorFactory[], ITypeTraits[]> getComparatorFactoriesAndTypeTraitsOfSecondaryBTreeIndex(
        ...
        if (indexType == IndexType.DYNAMIC_HILBERTVALUE_BTREE) {
            ...
            ++i;
            comparatorFactories[i] = AqlBinaryComparatorFactoryProvider.INSTANCE
                    .getBinaryComparatorFactory(BuiltinType.APOINT, true);
            ...
            comparatorFactories[i] = AqlBinaryComparatorFactoryProvider.INSTANCE
                    .getBinaryComparatorFactory(BuiltinType.ABINARY, true);
    ...
    private Pair<IOperatorDescriptor, AlgebricksPartitionConstraint> getBinaryTokenizerUpdateRuntime(
        ...
        try {
            ...
            if (indexType == IndexType.STATIC_HILBERT_BTREE) {
                ...
                tokenKeyPairTypeTraits[tokenOffset] = NonTaggedDataFormat.INSTANCE.getTypeTraitProvider()
                        .getTypeTrait(BuiltinType.ABINARY);
            } else {
                tokenKeyPairFields[tokenOffset] = serdeProvider
                        .getSerializerDeserializer(NonTaggedFormatUtil.getTokenType(secondaryKeyType));
    ...
    private Pair<IOperatorDescriptor, AlgebricksPartitionConstraint> getBinaryTokenizerQueryRuntime(
        ...
        IBinaryTokenizerFactory tokenizerFactory = new StaticHilbertBTreeBinaryTokenizerFactory(itp.bottomLeftX,
                ...
                new ByteArrayTokenFactory(ATypeTag.BINARY.serialize()),
                OptimizationConfUtil.getPhysicalOptimizationConfig().getFrameSize(), true);
        ...
        int numTokenKeyPairFields = numTokensPerOutputRecord
                + numKeys; /* 2 (= low key token + high key token) +  numKeys (= secondary key fields) */
        ISerializerDeserializer[] tokenKeyPairFields = new ISerializerDeserializer[numTokenKeyPairFields];
        ITypeTraits[] tokenKeyPairTypeTraits = new ITypeTraits[numTokenKeyPairFields];
        ...
        for (i = inputRecFieldCount; i < inputRecFieldCount + numTokensPerOutputRecord; i++) {
            tokenKeyPairFields[i] = NonTaggedDataFormat.INSTANCE.getSerdeProvider()
                    .getSerializerDeserializer(BuiltinType.ABINARY);
            tokenKeyPairTypeTraits[i] = NonTaggedDataFormat.INSTANCE.getTypeTraitProvider()
                    .getTypeTrait(BuiltinType.ABINARY);
        }
